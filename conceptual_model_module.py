...
def build_conceptual_model(
    ...
    """
    Build a conceptual model artifact from:
      - high-level simulation description (text)
      - available time-series columns (NetLogo output)
      - available parameter names (BehaviorSpace / NetLogo)
    The result is a software-independent conceptual model.
    """
    prompt = f"""
...
"""
    ...
    try:
        ...
        raise ValueError(
            f"LLM did not return valid JSON for conceptual model evaluation. Error: {e}\nRaw:\n{raw}"
        )

    # Clamp score into [0,1]
    s = float(eval_json.get("validity_score", 0.0))
    eval_json["validity_score"] = max(0.0, min(1.0, s))
